  ...
  return (
    <div>
      ...
      <RecordsSection
        ...
        mById={mById}
        completed={completed}
        positionCounts={completed.length ? positionCounts : { topId: undefined, botId: undefined }}
        topWeekly={completed.length ? topWeekly : null}
        biggestWin={completed.length ? biggestWin : null}
        highestFixture={completed.length ? highestFixture : null}
        longestWin={longestWin}
        longestLose={longestLose}
        dominantH2H={completed.length ? dominantH2H : null}
        mostCS={mostCS}
        mostGoals={mostGoals}
...
function FixturesPanel({ fixtures, managers, maxGW, seasonId }: any) {
  const completedGWs = [...new Set(
    fixtures.filter((f: any) => f.home_score != null && !(Number(f.home_score) === 0 && Number(f.away_score) === 0)).map((f: any) => f.gameweek)
  )] as number[];
  const latestGW = completedGWs.length ? Math.max(...completedGWs) : 1;
    ...
    <div className="mt-6">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {list.map((f: any) => {
          ...
          const hasScore = f.home_score != null && !(Number(f.home_score) === 0 && Number(f.away_score) === 0);
          const homeWon = hasScore && f.home_score > f.away_score;
          const awayWon = hasScore && f.away_score > f.home_score;
          const isDraw = hasScore && f.home_score === f.away_score;
